					  ...
					  ) 
{
	...
	//(although this is not fully the proper way to do this)
	...
	if (matches.size() <= 0) { 
        ...
        // the matches were obtained by BF-Matching, Common.cpp
        // so, index-align them based on matches.
        GetAlignedPointsFromMatch(imgpts1, imgpts2, matches, imgpts1_tmp, imgpts2_tmp); // output is index-aligned
	}
	...
	for (unsigned int i=0; i<status.size(); i++) {
		if (status[i]) 
		{
			...
			if (matches.size() <= 0) { //points already aligned...
                ...
                //
                // choose the matches[i], which is good and has the indexes for the original imgpts[]
    ...
    // if possible,
    // nonlinear optimization of F should be done here.
    //
	return F;
}
						...
						) 
{
    ...
    if(fabsf(determinant(E)) > 1e-07) {
        cout << "det(E) != 0 : " << determinant(E) << " return fasle!\n";
        P1 = 0;
        return false;
    }
    
    cerr << "det(E)= " << determinant(E) << " good enough.\n";
