...
void RichFeatureMatcher::MatchFeatures(int idx_i, int idx_j, vector<DMatch>* matches)
{
	...
	stringstream ss;
    ss << "imgpts1 has " << imgpts1.size() << " points (descriptors " << descriptors_1.rows << ")" << endl;
    cout << ss.str();
	stringstream ss1;
    ss1 << "imgpts2 has " << imgpts2.size() << " points (descriptors " << descriptors_2.rows << ")" << endl;
    cout << ss1.str();
    ...
    //
    //  do BF Matching of the descriptors
    //
    if (matches->size() == 0) {
    ...
    return; // now we may return. codes below are for testing.
// -------------------------------------------------------------
...
//    GetFundamentalMat(keypoints_1,keypoints_2,
//                      imgpts1_very_good,imgpts2_very_good,
//                      good_matches_);
//    // output: imgpts1_very_good, imgpts2_very_good, index aligned
//    //         good_matches_                      : matches of the two above
//    //Draw matches
//#ifdef __SFM__DEBUG__
//    {
//        //-- Draw only "good" matches
